...
static void UpdateSystemIdent(const float *X, const float *noise,
	...
	SystemIdentBetaToArray(relay.Beta)[SYSTEMIDENT_BETA_ROLL]    = X[6];
	SystemIdentBetaToArray(relay.Beta)[SYSTEMIDENT_BETA_PITCH]   = X[7];
	SystemIdentBetaToArray(relay.Beta)[SYSTEMIDENT_BETA_YAW]     = X[8];
	SystemIdentBiasToArray(relay.Bias)[SYSTEMIDENT_BIAS_ROLL]    = X[10];
	SystemIdentBiasToArray(relay.Bias)[SYSTEMIDENT_BIAS_PITCH]   = X[11];
	SystemIdentBiasToArray(relay.Bias)[SYSTEMIDENT_BIAS_YAW]     = X[12];
	relay.Tau                            = X[9];
	if (noise) {
		SystemIdentNoiseToArray(relay.Noise)[SYSTEMIDENT_NOISE_ROLL]  = noise[0];
		SystemIdentNoiseToArray(relay.Noise)[SYSTEMIDENT_NOISE_PITCH] = noise[1];
		SystemIdentNoiseToArray(relay.Noise)[SYSTEMIDENT_NOISE_YAW]   = noise[2];
	}
...
static void UpdateStabilizationDesired(bool doingIdent) {
	...
	StabilizationBankData stabBankSettings;
	StabilizationBankGet(&stabBankSettings);
	...
	stabDesired.Roll = manualControl.Roll * stabBankSettings.RollMax;
	stabDesired.Pitch = manualControl.Pitch * stabBankSettings.PitchMax;
	stabDesired.Yaw = manualControl.Yaw * stabBankSettings.ManualRate.Yaw;
	stabDesired.Thrust = manualControl.Throttle;
	...
	ManualControlCommandThrottleGet(&stabDesired.Thrust);
